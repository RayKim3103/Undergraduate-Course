...
public class WarriorBasicAttack : MonoBehaviour
{
    /* Player 정보를 담을 변수 */
    ...
    /* BasicSkill 담을 변수 */
    private WarriorBasicSkillEffect warriorBasicSkillEffect;
    ...
    private void Awake() 
    {
        ...
        // 시간(초) 단위로 사라지게 만들지 말고 일정 범위 벗어날 시 사라지게 만들어주셔요
        // DestroyAuto.cs 참고하면 됨
    ...
    private void HandleEnemy(GameObject target)
    {
        ...
        if (enemy != null)
        {
            ...
            // 적에서 MageBasicSkillEffect 컴포넌트를 가져옴
            // warriorBasicSkillEffect = enemy.GetComponent<WarriorBasicSkillEffect>();
            ...
            // warriorBasicSkillEffect.BasicSkill();
