...
//  assign led_out = data[7:0];
  ...
  begin
    ...
    begin
      if (clk_led == 32'd50000000)
      begin
        clk_led <= 32'd0;
        if (led_cnt == 3) led_cnt <= 2'd0;
        else led_cnt <= led_cnt + 2'd1;
      end
      else
      begin
        clk_led <= clk_led + 32'd1;
      end
    end
  end
  ...
  begin
    case (led_cnt)
      2'd0: begin
        led_out = leddata[7:0]; end
      2'd1: begin
        led_out = leddata[15:8]; end
      2'd2: begin
        led_out = leddata[23:16]; end
      2'd3: begin
        led_out = leddata[31:24]; end
    endcase
  end
  //////
  always @ (negedge resetn or posedge clk)
