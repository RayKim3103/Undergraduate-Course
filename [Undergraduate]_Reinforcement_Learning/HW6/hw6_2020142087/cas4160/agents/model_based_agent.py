...
class ModelBasedAgent(nn.Module):
    ...
    def evaluate_action_sequences(self, obs: np.ndarray, action_sequences: np.ndarray):
        ...
        for step in range(action_sequences.shape[1]):
            ...
            ####################
            #next_obs = ...
            next_obs = np.stack([self.get_dynamics_predictions(e, obs[e], acs) for e in range(self.ensemble_size)])
            ####################
    ...
    def get_action(self, obs: np.ndarray):
        ...
        if self.mpc_strategy == "random":
            ...
            for i in range(self.cem_num_iters):
                ...
                elite_sequences = action_sequences[elite_idxs]
                
                mean = np.mean(elite_sequences, axis=0)
                std = np.std(elite_sequences, axis=0)
                
                action_sequences = np.random.normal(mean, std, size=action_sequences.shape)
                ####################
