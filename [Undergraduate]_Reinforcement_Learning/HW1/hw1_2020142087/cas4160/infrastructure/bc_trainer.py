...
class BCTrainer:
    ...
    def collect_training_trajectories(
        ...
        if itr == 0 and load_initial_expertdata is not None:
            # 첫 번째 iter이고 expert data 파일 있으면 load
            with open(load_initial_expertdata, 'rb') as f:
                trajs = pickle.load(f)
            return trajs, 0, None
        
        # rollout_trajectories로 data 수집 (itr==0 but no expert, or itr>0)
    ...
    def do_relabel_with_expert(self, expert_policy, trajs):
        ...
        for i in range(len(trajs)):
            ...
            expert_acs = expert_policy.get_action(obs)  # get_action으로 expert action 얻기
