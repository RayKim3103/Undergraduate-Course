...
class BCTrainer:
    ...
    def collect_training_trajectories(
        ...
        if itr == 0 and load_initial_expertdata is not None:
            # 첫 번째 iter이고 expert data 파일 있으면 로드
            with open(load_initial_expertdata, 'rb') as f:
                trajs = pickle.load(f)
            return trajs, 0, None
        
        # rollout_trajectories로 데이터 수집 (itr==0 but no expert, or itr>0)
        batch_size = self.params['batch_size_initial'] if itr == 0 else self.params['batch_size']
        trajs, envsteps_this_batch = utils.rollout_trajectories(
            self.env,
            collect_policy,
            min_timesteps_per_batch=batch_size,
            max_traj_length=self.params['ep_len'],
            render=False  # 학습 데이터 수집 시 render off
        )
        ###########
    ...
    def do_relabel_with_expert(self, expert_policy, trajs):
        ...
        for i in range(len(trajs)):
            obs = trajs[i]["observation"]  # (len_traj, ob_dim) numpy array
            expert_acs = expert_policy.get_action(obs)  # get_action으로 expert 행동 얻기 (numpy array)
            trajs[i]["action"] = expert_acs  # 기존 action 교체
        ##########
