...
def build_mlp(
    ...
    ############
    # input layer: nn.Linear(input_size, size) + activation()
    # hidden layers: 각 nn.Linear(size, size) + activation()  -->  (n_layers - 1개)
    # output layer: nn.Linear(size, output_size) + output_activation()
    
    layers = []
    # Input to first hidden layer
    current_input = input_size
    if n_layers > 0:
        layers.append(nn.Linear(current_input, size))
