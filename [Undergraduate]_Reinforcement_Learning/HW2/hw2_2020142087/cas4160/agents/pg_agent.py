...
class PGAgent(nn.Module):
    ...
    def update(
        ...
        actions = np.concatenate(actions, axis=0)   # shape: (batch_size, ac_dim) or (batch_size,)
        ...
        if not self.use_ppo:
            ...
            if self.normalize_advantages:
                # pass
                ###########
                advantages = (advantages - np.mean(advantages)) / (np.std(advantages) + 1e-8) # std가 0이 되는 경우를 방지 -> 안 하니 Error 발생했음
                ###########
            ...
            if self.critic is not None:
                ...
                for _ in range(self.baseline_gradient_steps):
                    critic_info_step = self.critic.update(obs, q_values)
                    for k, v in critic_info_step.items():
                        critic_info[k] = critic_info.get(k, 0) + v
                ###########
            ...
            for _ in range(self.n_ppo_epochs):
                ...
                for start in range(0, n_batch, minibatch_size):
                    ...
                    if self.normalize_advantages:
                        # pass
                        ###########
                        advantages_slice = (advantages_slice - np.mean(advantages_slice)) / (np.std(advantages_slice) + 1e-8) # std가 0이 되는 경우를 방지 -> 안 하니 Error 발생했음
                        ###########
    ...
    def _discounted_return(self, rewards: np.ndarray[float]) -> np.ndarray[float]:
        ...
        for i in reversed(range(len(rewards) - 1)):
            ret[i] = rewards[i] + self.gamma * ret[i + 1]

        # 전체 trajectory에 대해 같은 값이므로 broadcast
        ret = np.full_like(rewards, ret[0], dtype=float)
        ###########
    ...
    def _calculate_log_probs(
        ...
        # actor는 distribution을 반환 -> log_prob() 사용
