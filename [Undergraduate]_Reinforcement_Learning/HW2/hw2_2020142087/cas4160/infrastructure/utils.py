...
def rollout_trajectory(
    ...
    while True:
        ...
        # ac: np.ndarray = None
        ###########
        ob_t = ptu.from_numpy(np.expand_dims(ob, axis=0))   # shape: (1, ob_dim)
        dist = policy(ob_t)                                 # MLPPolicyPG는 distribution 반환
        ac = ptu.to_numpy(dist.sample()).squeeze(0)         # shape: (ac_dim,)
        ###########
