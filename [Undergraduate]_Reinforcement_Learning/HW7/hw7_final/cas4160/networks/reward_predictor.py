...
class RewardPredictor(nn.Module):
    ...
    def predict_rewards(self, obs: np.ndarray, acts: np.ndarray, training=False) -> np.ndarray:
        ...
        inputs_flat = torch.cat([obs_flat, acts_flat], dim=1)
        rewards_flat = self.reward_net(inputs_flat).squeeze(-1)
        # rewards_flat = None
        ##########
    ...
    def predict_preferences(self, obs1, acts1, obs2, acts2):
        ...
        reward_sum_1 = reward_1.sum(dim=1)
        reward_sum_2 = reward_2.sum(dim=1)

        logits = torch.stack([reward_sum_1, reward_sum_2], dim=1)
    ...
    def compute_loss_and_accuracy(self, obs1, acts1, obs2, acts2, prefs):
        ...
        loss = -(prefs * torch.log(preds + 1e-8)).sum(dim=1).mean()
        # preds = None
        # loss = None
        ##########
    ...
    def train_step(self, obs1, acts1, obs2, acts2, prefs):
        ...
        loss, accuracy = self.compute_loss_and_accuracy(
            obs1, acts1, obs2, acts2, prefs
        )
