    ...
    for itr in range(args.n_iter):
        ...
        if itr % args.annotate_freq == 0:
            ...
            for annotated_traj in annotated_traj_list:
                ...
                # segment_len = int(fps * 2)를 쓰는 이유:
                # annotation GUI에서 마지막 약 2초짜리 video segment를 보여주기 때문
                segment_len = min(
                    len(traj1["observation"]),
                    len(traj2["observation"]),
                    int(fps * 2),
                )

                obs1 = traj1["observation"][-segment_len:]
                acts1 = traj1["action"][-segment_len:]
                obs2 = traj2["observation"][-segment_len:]
                acts2 = traj2["action"][-segment_len:]
                prefs = np.array([prefs], dtype=np.float32)

                if not hasattr(replay_buffer, "obs1"):
                    replay_buffer.obs1 = []
                    replay_buffer.acts1 = []
                    replay_buffer.obs2 = []
                    replay_buffer.acts2 = []
                ...
                replay_buffer.obs1.append(obs1)
                replay_buffer.acts1.append(acts1)
                replay_buffer.obs2.append(obs2)
                replay_buffer.acts2.append(acts2)
                replay_buffer.prefs.append(prefs)
                ##########
            ...
            for _ in range(num_update_reward_predictor):
                ...
                # batch = None
                # loss, accuracy = None
                num_preferences = len(replay_buffer.prefs)
                sample_size = min(batch_size_reward_predictor, num_preferences)

                indices = np.random.choice(
                    num_preferences,
                    size=sample_size,
                    replace=(num_preferences < batch_size_reward_predictor),
                )
                ...
                # prefs : (B, 1)
                batch = (
                    np.stack([replay_buffer.obs1[i] for i in indices], axis=0),
                    np.stack([replay_buffer.acts1[i] for i in indices], axis=0),
                    np.stack([replay_buffer.obs2[i] for i in indices], axis=0),
                    np.stack([replay_buffer.acts2[i] for i in indices], axis=0),
                    np.stack([replay_buffer.prefs[i] for i in indices], axis=0),
                )

                loss, accuracy = agent.reward_predictor.train_step(*batch)
                ##########
