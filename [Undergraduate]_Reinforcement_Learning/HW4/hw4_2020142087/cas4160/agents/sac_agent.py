...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            ...
            # raise NotImplementedError
            min_next_q = torch.min(next_qs, dim=0, keepdim=True)[0]
            next_qs = min_next_q.expand(self.num_critic_networks, batch_size)
            ####################
        else:
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            next_action = next_action_distribution.sample()   # target이므로 .sample() 사용
            ####################
            ...
            if self.use_entropy_bonus and self.backup_entropy:
                ...
                next_qs = next_qs + self.temperature * next_action_entropy.unsqueeze(0)
                ####################
            ...
            target_values: torch.Tensor = (
                reward[None, :]
                + self.discount * (1 - done.float()[None, :]) * next_qs 
            )
            assert target_values.shape == (
                self.num_critic_networks,
                batch_size
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        # action = ...
        # entropy = ...
        action = action_distribution.rsample()          # reparametrization과 호환되도록 rsample 사용
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            #     batch_size,
            # ), q_values.shape
            obs_expanded = obs.unsqueeze(0).expand(self.num_actor_samples, -1, -1)
