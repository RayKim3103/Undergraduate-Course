...
class DQNAgent(nn.Module):
    ...
    def get_action(self, observation: np.ndarray, epsilon: float = 0.02) -> int:
        ...
        with torch.no_grad():
            ...
            if np.random.rand() < epsilon:
                action = torch.randint(0, self.num_actions, (1,))   # random action
            else:
                action = qa_values.argmax(dim=1)                    # greedy action
        ####################
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            # target_values = ...
            next_qa_values = self.target_critic(next_obs)           # always use target network for value estimation

            if self.use_double_q:
                # Choose action with argmax of critic network (online)
        ...
        q_values = qa_values.gather(1, action.unsqueeze(1)).squeeze(1)   # select Q(s, a)
